const width = 750; const height = 500; const r = 175; const legendSize = 100;
  ...
  return (
    <svg className="pie-chart" viewBox={`0 0 ${width} ${height}`}>
      ...
      <g className="legend">
        {
          data.map ((d, i) => (
            <text textAnchor="middle" x={width / 2} y={(legendSize * (i - (data.length - 1) / 2) / ((data.length - 1) / 2)) + height / 2}>{d.label}</text>
          ))
        }
      </g>
    </svg>
